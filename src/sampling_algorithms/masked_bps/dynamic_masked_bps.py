...
class SubMaskedLocalBPS(LinearPDMCMC):
    ...
    def __init__(self, handles, factor_group=None):
        x_handle, v_handle, t_handle, mask_handle, fg_index_handle, factor_graph_handle, bounce_fns_handle = handles
        ...
        # store global facto graph lists, once
        self.global_factor_graph_list = ray.get(factor_graph_handle)
        self.global_bounce_fns_list = ray.get(bounce_fns_handle)
    ...
    def re_init(self, handles, factor_group):
        ...
        self.fg_index = ray.get(fg_index_handle)

        # load globals
        self.global_factor_graph = self.global_factor_graph_list[self.fg_index]
        self.global_bounce_fns = self.global_bounce_fns_list[self.fg_index]
        ...
        self.factor_graph = FactorGraph(dim_x=self.d,
                                        ...
                                        grad_factor_potential_fns=[
                                            self.global_factor_graph.grad_factor_potential_fns[factor]
                                            for factor in factor_group])
    ...
    def propagate_factor_x(self, f, new_t):
        ...
        x_new[factor_ind] = self.x[factor_ind].copy() + (new_t - self.t[factor_ind].copy()) * effective_v[
            factor_ind].copy()
...
class DynamicMaskedLocalBPS(object):
    def __init__(self, init_x,
        ...
        # factor graph
        self.factor_graph_list = factor_graph_list
        self.bounce_fn_list = bounce_fn_list
        self.factor_graph = factor_graph_list[init_fg]

        self.factor_graph_handle = ray.put(factor_graph_list)
        self.bounce_fns_handle = ray.put(bounce_fn_list)

        # mask fns
        self.sample_graph_and_mask_fn = sample_graph_and_mask_fn
    ...
    def share_params(self):
        ...
        self.fg_index_handle = ray.put(self.fg_index)

    def get_handles(self):
        return self.x_handle, self.v_handle, self.t_handle, self.mask_handle, self.fg_index_handle, self.factor_graph_handle, self.bounce_fns_handle

    def get_state(self):
        return self.x.copy(), self.v.copy(), self.t.copy(), self.mask.copy(), self.fg_index
    ...
    def refresh_event(self):
        ...
        fg_index, mask = self.sample_graph_and_mask_fn()
        self.mask = mask
        self.fg_index = fg_index
        self.factor_graph = self.factor_graph_list[fg_index]
        ...
        new_x = np.array(ray.get([
            self.sub_samplers[i].run_to_T.remote(handles,
                                                 ...
                                                 os.path.join(output_dir,
                                                              "sub_results_iter{0}_group{1}.pickle".format(iteration,
                                                                                                           i)))
